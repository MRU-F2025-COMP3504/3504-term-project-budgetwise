 ...
 * useQuiz Hook
 *
 * This custom hook manages the entire lifecycle of the AI onboarding quiz.
 * It handles:
 * 1. Sending your chat history to the server.
 * 2. Getting the next question from the AI.
 * 3. Tracking loading states and errors.
 * 4. Handling the final results when the quiz is done.
 *
 * It's like a mini-state machine for the quiz UI.
 */
...
export default function useQuiz({ apiPath = "/api/quiz", onComplete } = {}) {
  ...
  const submitAnswer = useCallback(
    async (answer, q = currentQ) => {
      if (!q || !answer) return;
      const userTurn = {
        role: "user",
        content: `Answer to ${q.id}: ${answer}`,
      };
