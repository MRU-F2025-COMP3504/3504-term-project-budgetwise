...
function LayoutContent({ children }) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[var(--background)] flex items-center justify-center text-[var(--color-text-muted)]">
        Loading...
      </div>
    );
  }
      ...
      <div className="flex min-h-screen bg-[var(--background)]">
        ...
        <div className="md:hidden fixed top-0 left-0 right-0 h-16 bg-[var(--card-bg)] border-b border-[var(--color-border)] flex items-center px-4 z-40">
          <button
            ...
            className="p-2 text-[var(--color-text)] hover:bg-[var(--surface-raised)] rounded-md transition-colors"
          >
            <Menu size={24} />
          </button>
          <span className="ml-4 font-bold text-xl text-[var(--color-text)]">
            BudgetWise
          </span>
        </div>
        ...
        <main
          className={`flex-1 p-4 md:p-8 transition-all duration-300 pt-20 md:pt-8 ${isCollapsed ? "md:ml-20" : "md:ml-64"}`}
        >
          {children}
        </main>
        <AICompanion />
