...
export default function UploadPage() {
  ...
  const handleFileSelection = (e) => {
    ...
    files.forEach(file => {
      initialResults[file.name] = { status: 'pending', message: 'Ready to upload' };
    });
  ...
  const uploadAllFiles = async (filesToUpload = selectedFiles) => {
    ...
    const allSuccessful = Object.values(newResults).every(result => result.status === 'ok');
    setStatusMessage(
      allSuccessful 
      ...
      <div className="bw-card p-6 flex flex-col">
        ...
        <div className="border-2 border-dashed border-[rgba(255,255,255,0.1)] rounded-xl p-5 w-full flex flex-col gap-4">
          ...
          {selectedFiles.length > 0 && (
            <div className="text-xs bw-text-muted space-y-1">
              <p className="font-semibold">Selected ({selectedFiles.length}):</p>
              <ul className="list-disc list-inside space-y-0">
                {selectedFiles.map(file => (
                  <li key={file.name} className="flex justify-between items-center gap-2">
          ...
          {selectedFiles.length > 0 && (
            <button
              ...
              aria-label={isUploading ? 'Uploading files' : `Upload ${selectedFiles.length} file(s)`}
            >
              {isUploading 
                ? 'Uploading...' 
        ...
        <div className="mt-4 space-y-2 text-xs">
          ...
          {!selectedFiles.length && (
            <p className="text-[var(--color-text-muted)]">No files selected yet.</p>
          )}
        </div>
