export default function Table({
  ...
  // 1. Auto-Generate Columns
  // If no columns are provided, we look at the data and make a column for every key.
  ...
  const isDateish = (key, v) =>
    ...
    /date|time|created|updated|period/i.test(key);
          ...
          <thead>
            <tr className="bg-[var(--color-surface-2)] text-left">
              {colDefs.map((col) => (
                <th
                  key={col.key}
                  className={`px-4 py-3 font-medium text-[var(--color-text-muted)] whitespace-nowrap ${col.className || ""}`}
                >
                  {col.label}
                </th>
              ))}
