    ...
    <>
      ...
      `}>
        ...
        >
          ...
          <span className={`font-bold text-xl tracking-tight text-[var(--color-text)] animate-in fade-in duration-200 ${isCollapsed ? 'md:hidden' : 'block'}`}>
            BudgetWise
          </span>
        </Link>
        ...
        <nav className="flex-1 px-2 space-y-1 overflow-y-auto py-4">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <Icon size={20} className={`shrink-0 ${isActive ? "text-white" : "text-[var(--color-text-muted)] group-hover:text-[var(--color-text)] transition-colors"}`} />
                <span className={`font-medium text-sm animate-in fade-in duration-200 whitespace-nowrap ${isCollapsed ? 'md:hidden' : 'block'}`}>
                  {item.label}
                </span>
              </Link>
            );
        ...
        {/* User Profile */}
        <div className={`border-t border-[var(--color-border)] bg-[var(--surface-raised)]/30 ${isCollapsed ? 'md:p-2' : 'p-4'} p-4 relative`}>
          <button 
          ...
          >
            ...
            <div className={`flex-1 min-w-0 animate-in fade-in duration-200 ${isCollapsed ? 'md:hidden' : 'block'}`}>
