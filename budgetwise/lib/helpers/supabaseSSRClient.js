...
/**
 * Returns a request-scoped Supabase client using cookies managed by Next.js.
 * Prefer this for SSR/API routes over manual bearer headers.
 */
export async function getSupabaseServerClient() {
  ...
  if (!url || !anonKey) {
    throw new Error(`Supabase env vars missing. url=${!!url} anonKey=${!!anonKey}`);
  }
    ...
    cookies: {
      ...
      remove(name, options) {
        try {
          ...
          try { cookieStore.set({ name, value: '', ...options, maxAge: 0 }); } catch (_) {}
        }
      },
    },
  });
